...
def main():
    ...
    conn = sqlite3.connect(DB_PATH)
    spy_df = pd.read_sql("SELECT date,close FROM ohlcv_cache WHERE ticker='SPY' ORDER BY date", conn)
    spy_df["date"] = pd.to_datetime(spy_df["date"].str[:10]); spy_df = spy_df.set_index("date")
    vix_df = pd.read_sql("SELECT date,close FROM ohlcv_cache WHERE ticker='^VIX' ORDER BY date", conn)
    if vix_df.empty:
        vix_df = pd.read_sql("SELECT date,close FROM ohlcv_cache WHERE ticker='VIXY' ORDER BY date", conn)
    vix_df["date"] = pd.to_datetime(vix_df["date"].str[:10]); vix_df = vix_df.set_index("date")
    conn.close()
    ...
    trading_days = pd.date_range(args.start, args.end, freq="B")
    ...
    print(f"  Blocked days: {sum_df[sum_df['status'].str.contains('BLOCKED')]['status'].count()}")
    print(f"  Active days: {sum_df[~sum_df['status'].str.contains('BLOCKED')]['status'].count()}")
