...
def run_period(label: str, start_date: str, end_date: str, threshold: Optional[float]) -> dict:
    logger.info("Running %s with rs_breakout_min=%s", label, threshold)
...
def main() -> None:
    ...
    payload = {
        "experiment": "rs_breakout_filter",
        "date": datetime.now().strftime("%Y-%m-%d"),
        "hypothesis": "RS percentile alto en el breakout predice menos fallas",
        "variables_modified": ["rs_breakout_min"],
    ...
    output_path = output_dir / f"rs_breakout_sandbox_{ts}.json"
