...
def run_engine(start_date: str, end_date: str, threshold: Optional[float]) -> dict:
    ...
    engine = AdvancedVectorBTEngine(
        ...
        rvol_breakout_threshold=threshold,
        offline_mode=True,
    )
    return engine.run_backtest()


def run_period(label: str, start_date: str, end_date: str, threshold: Optional[float]) -> dict:
    logger.info("Running %s with threshold=%s", label, threshold)
...
def main() -> None:
    ...
    payload = {
        "experiment": "rvol_breakout_filter",
        "date": datetime.now().strftime("%Y-%m-%d"),
        "hypothesis": "RVOL > threshold en dia del breakout filtra roturas falsas",
        "variables_modified": ["rvol_breakout_threshold"],
        "variables_fixed": ["min_rvol", "tp1_r", "tp2_r", "stop_atr", "all_exits"],
        "dataset": {
            "is_start": IS_START,
            "is_end": IS_END,
            "oos_start": OOS_START,
        ...
        "baseline": "combo_pure_momentum — sharpe_wf=1.72, dd=18%",
    ...
    if args.mode == "wf":
        ...
        payload["notes"] = "Walk-forward uses 6m IS + 3m OOS windows stepped every 3m."

    OUTPUT_DIR = PROJECT_ROOT / "outputs" / "experiments"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = OUTPUT_DIR / f"rvol_breakout_sandbox_{ts}.json"
