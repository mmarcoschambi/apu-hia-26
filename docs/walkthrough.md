...
La arquitectura se encuentra 100% enfocada en **Docker & Docker Compose**:
...
## 📁 Estructura Limpia del Repositorio (100% Scrum & Docker)

```
D:\FACUer_año\HIA-worktrees	p-scrum├── docker-compose.yml                      <-- Orquestación base con internal: true y aliases DNS
├── docker-compose.override.yml.example     <-- Plantilla para desarrollo local (127.0.0.1:3306)
├── .env.example                            <-- Plantilla de variables seguras
├── .gitignore                              <-- Exclusión estricta de secrets/, certs/ y overrides
├── nginx/
