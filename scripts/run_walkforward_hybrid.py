...
def compute_metrics(trades: list[dict]) -> dict:
    if not trades:
        return {
            ...
            "profit_factor_defined": False, "sharpe_defined": False, "low_sample": True,
        }
    ...
    return {
        ...
        "win_rate": round(wr, 3), "total_pnl": round(sum(pnls), 2),
        "profit_factor": pf, "sharpe": round(sharpe, 3),
        "max_drawdown": round(max_dd, 4), "expectancy": round(expectancy, 4),
        "avg_win": round(avg_win, 2), "avg_loss": round(avg_loss, 2),
        "profit_factor_defined": pf_defined, "sharpe_defined": sharpe_defined,
        "low_sample": len(trades) < 15,
    }


def _load_combos_for_mode(mode: SignalMode, combo_source: str, experiment_overrides: dict | None = None) -> tuple[dict, dict] | dict:
    def _load(name: str) -> dict:
        loaded = load_combo_merged(name) if combo_source == "merged" else (load_combo_base_only(name), None)
        cfg = loaded[0]
        # Overrides Globales: Se aplican a cualquier modo (A o B) si se pasan
        if experiment_overrides:
            if "screener" not in cfg: cfg["screener"] = {}
            if "params" not in cfg["screener"]: cfg["screener"]["params"] = {}
            if "tier2_filters" not in cfg: cfg["tier2_filters"] = {}
            
            for k, v in experiment_overrides.items():
                # 1. Filtros de Régimen y Tier2
                if k in ["require_spy_above_sma50", "require_spy_above_sma200", "min_rs_percentile", "min_rvol", "min_adr"] or k in cfg["tier2_filters"]:
...
def generate_signals_over_window(
    universe: list[str], df_cache: dict, spy_df: pd.DataFrame, mode: SignalMode,
    start_date: str, end_date: str, skip_tier2: bool, combo_source: str, overrides: dict | None,
    phase: Literal["is", "oos"]
) -> tuple[list[SignalDecision], dict]:
    all_decisions = []
    funnel = {"n_evaluations": 0, "n_min_data_fail": 0, "n_screener_fail": 0, "n_tier2_fail": 0, "n_passed": 0}
    
    window_days = spy_df.index[(spy_df.index >= start_date) & (spy_df.index <= end_date)]
    if mode == "A_BOTH": cfg_a, cfg_b = _load_combos_for_mode(mode, combo_source, overrides)
    else: cfg = _load_combos_for_mode(mode, combo_source, overrides)
    ...
    for current_dt in window_days:
        ...
        for ticker in universe:
            ...
            if df_source is None or current_dt not in df_source.index: continue
            ...
            if mode == "A":
                d = evaluate_ticker(ticker=ticker, df=df_pit, spy_df=spy_pit, combo_cfg=cfg, mode="A", skip_tier2=skip_tier2, scan_date=curr_str)
            elif mode == "B":
                d = evaluate_ticker(ticker=ticker, df=df_pit, spy_df=spy_pit, combo_cfg=cfg, mode="B", skip_tier2=skip_tier2, scan_date=curr_str)
            else:
                da = evaluate_ticker(ticker=ticker, df=df_pit, spy_df=spy_pit, combo_cfg=cfg_a, mode="A", skip_tier2=skip_tier2, scan_date=curr_str)
                db = evaluate_ticker(ticker=ticker, df=df_pit, spy_df=spy_pit, combo_cfg=cfg_b, mode="B", skip_tier2=skip_tier2, scan_date=curr_str)
            ...
            if d.passed:
                ...
                if "screener_fail" in reason: funnel["n_screener_fail"] += 1
                elif "tier2_fail" in reason: funnel["n_tier2_fail"] += 1
    return all_decisions, funnel


def simulate_trades_from_signals(signals: list[SignalDecision], df_cache: dict) -> list[dict]:
    trades = []
    active_positions = {}
    ...
    for s in signals:
        ...
        if ticker in active_positions and ts_signal <= active_positions[ticker]: continue
        ...
        try:
            ...
            if idx_sig + 1 >= len(df_source): continue
            ...
            trades.append({
                "ticker": ticker, "mode": s.mode, "signal_date": signal_date,
                "entry_date": str(df_source.index[idx_ent].date()), 
                "exit_date": str(df_source.index[idx_ext].date()),
                "entry_price": ent_p, "exit_price": ext_p, "pnl": round(pnl, 2),
...
def run_fold(fold_idx, is_start, is_end, oos_start, oos_end, universe, mode, dry_run, skip_tier2, combo_source, snapshot, overrides, debug_trades=False) -> dict:
    logger.info(f"  FOLD {fold_idx}: OOS {oos_start} -> {oos_end}")
    if dry_run: return {"fold": fold_idx, "mode": mode, "gate": {"verdict": "HOLD", "reasons": ["dry_run"]}}
    ...
    for t in universe:
        try:
            ...
            df_cache[t] = {"full_is": load_ohlcv(t, fsi, is_end), "full_oos": load_ohlcv(t, fso, oos_end)}
        except Exception: continue

    is_sigs, is_fun = generate_signals_over_window(universe, df_cache, is_spy, mode, is_start, is_end, skip_tier2, combo_source, overrides, phase="is")
    oos_sigs, oos_fun = generate_signals_over_window(universe, df_cache, oos_spy, mode, oos_start, oos_end, skip_tier2, combo_source, overrides, phase="oos")
    ...
    if debug_trades and oos_trades:
        ...
        for t in oos_trades[:15]:
            logger.info(f"      {t['ticker']} | Sig: {t['signal_date']} | Ent: {t['entry_date']} @ {t['entry_price']:.2f} | Ext: {t['exit_date']} @ {t['exit_price']:.2f} | PnL: ${t['pnl']}")
        if len(oos_trades) > 15: logger.info(f"      ... and {len(oos_trades) - 15} more.")

    gate_v = "HOLD"
    if oos_m["trades"] >= DEFAULT_MIN_OOS_TRADES:
        if (oos_m["profit_factor"] is None or oos_m["profit_factor"] >= 1.2) and oos_m["win_rate"] >= 0.35: gate_v = "PROMOTE"
        else: gate_v = "REJECT"

    res = {
        "fold": fold_idx, "mode": mode, "is_start": is_start, "is_end": is_end, "oos_start": oos_start, "oos_end": oos_end,
        "is_metrics": is_m, "oos_metrics": oos_m, "gate": {"verdict": gate_v},
        "funnel": {"is": is_fun, "oos": oos_fun}
    }
    if snapshot: res["universe"] = {"n_tickers": snapshot.n_selected, "tickers": snapshot.tickers}
    
    logger.info(f"    IS: {is_m['trades']} trades (PF={_fmt_metric(is_m['profit_factor'])}) | OOS: {oos_m['trades']} trades (PF={_fmt_metric(oos_m['profit_factor'])}) | GATE: {gate_v}")
    return res
...
def main():
    ...
    parser.add_argument("--override-a-require-spy-above-sma50", choices=["true", "false"])
    parser.add_argument("--override-a-require-spy-above-sma200", choices=["true", "false"])
    args = parser.parse_args()

    overrides = {}
    if args.override_a_min_rs_percentile: overrides["min_rs_percentile"] = args.override_a_min_rs_percentile
    if args.override_a_min_trend_intensity: overrides["min_trend_intensity"] = args.override_a_min_trend_intensity
    if args.override_a_min_adr_pct: overrides["min_adr_pct"] = args.override_a_min_adr_pct
    if args.override_a_require_ma_stack: overrides["require_ma_stack"] = (args.override_a_require_ma_stack == "true")
    if args.override_a_require_spy_above_sma50: overrides["require_spy_above_sma50"] = (args.override_a_require_spy_above_sma50 == "true")
    if args.override_a_require_spy_above_sma200: overrides["require_spy_above_sma200"] = (args.override_a_require_spy_above_sma200 == "true")
    ...
    for i in range(1, args.folds):
        ...
        if oos_e > e_dt: oos_e = e_dt
        folds.append((args.start, is_e.strftime("%Y-%m-%d"), oos_s.strftime("%Y-%m-%d"), oos_e.strftime("%Y-%m-%d")))
    ...
    for mode in args.modes:
        ...
        for i, (is_s, is_e, oos_s, oos_e) in enumerate(folds):
            ...
            f_results.append(run_fold(i, is_s, is_e, oos_s, oos_e, snap.tickers, mode, args.dry_run, args.skip_tier2, args.combo_source, snap, overrides, args.debug_trades))
    ...
    with open(out_dir / "walkforward_report.json", "w") as f:
        json.dump({"generated_at": datetime.now().isoformat(), "results": all_results}, f, indent=2, default=str)
