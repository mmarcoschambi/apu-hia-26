...
8. Suggest next steps. Ask the user what do they want to do:
   ...
   - Implement the plan by running the OpenSpec slash command `/sdd-ff` (or `/sdd-new` for the generated phase) in the repository (implements Phase 1 only).
   - Implement a specific phase by running `/sdd-new` (or `/sdd-ff`) directly on that phase in the repository.

   > [!IMPORTANT]
   > `/sdd-ff` and `/sdd-new` handle one phase per invocation. Never implement all phases at once.
