...
class BacktestVisualizer:
    ...
    def visualize_trade(self, symbol: str, entry_date: str, signal_data: dict, 
        ...
        end_idx = min(len(daily_df), entry_idx + days_after)
        ...
        # Main price chart
        self._plot_price_chart(ax1, window_df, signal_data, avwap_data)
        
        # Volume chart
        self._plot_volume_chart(ax2, window_df, entry_date_pd)
    ...
    def _plot_price_chart(self, ax, df, signal_data, avwap_data):
        ...
        if entry_price:
            ax.scatter(entry_idx, entry_price, color='lime', s=200, marker='^', 
    ...
    def _plot_volume_chart(self, ax, df, entry_date):
        """Plot volume bars"""
        x = np.arange(len(df))
        ...
        # Entry marker - find closest date in window
        try:
            entry_date = pd.to_datetime(entry_date).tz_localize(None)
