...
class MarketRegimeClassifier:
    ...
    def _precompute_indicators(self):
        ...
        # Relative volatility
        self.spy["range_pct"] = (
            (self.spy["high"] - self.spy["low"]) / self.spy["close"] * 100
        )
    ...
    def get_regime_mask(
        self, target_index: pd.DatetimeIndex, regime_type: str = "bullish_spy"
    ) -> pd.Series:
        """
        ...
        aligned_vix = (
            self.vix.reindex(target_index, method="ffill")
            if self.vix is not None
            else None
        )
        ...
        if regime_type == "bullish_spy":
            ...
            trend = (
                (aligned_spy["close"] > aligned_spy["sma200"])
    ...
    def get_market_stage(self, date: pd.Timestamp) -> str:
        ...
        if (
            spy_price < row["sma200"]
            and spy_price < row["sma50"]
            and row["mom_20"] < -0.05
        ):
            return "STAGE_4"
...
def load_spy_vix_data(
    start_date: str, end_date: str, cache=None, offline: bool = False
) -> tuple:
    """
    ...
    Args:
        ...
        offline: If True, only use cached data, never download from yfinance
    ...
    if cache is not None:
        try:
            if hasattr(cache, "get_daily_data"):
                spy_data = cache.get_daily_data(
                    "SPY", start_date=start_date, end_date=end_date, offline=offline
                )
    ...
    try:
        if cache is not None:
            if hasattr(cache, "get_daily_data"):
                vix_data = cache.get_daily_data(
                    "^VIX", start_date=start_date, end_date=end_date, offline=offline
                )
                ...
                vix_data = cache.get_ohlcv(
                    "^VIX", start_date, end_date, offline=offline
                )
                if vix_data is None or vix_data.empty:
                    vix_data = cache.get_ohlcv(
                        "VIX", start_date, end_date, offline=offline
