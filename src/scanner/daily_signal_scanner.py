...
def get_market_context():
    ...
    ctx = {"spy_ok": True, "vix_ok": True, "spy_price": None,
           "spy_sma50": None, "vix": None}
    try:
        spy = yf.download("SPY", period="60d", auto_adjust=True,
                          progress=False, timeout=10)
        if spy is not None and not spy.empty:

            c = spy["Close"].squeeze()
            ctx["spy_price"] = float(c.iloc[-1])
            ctx["spy_sma50"] = float(c.rolling(50).mean().iloc[-1])
            ctx["spy_ok"] = ctx["spy_price"] >= ctx["spy_sma50"] if REQ_SPY_SMA50 else True
    ...
    try:
        ...
        if vix is not None and not vix.empty:
            ctx["vix"] = float(vix["Close"].squeeze().iloc[-1])
            ctx["vix_ok"] = ctx["vix"] < MAX_VIX
    except Exception as e:
...
def scan_ticker(ticker, df, rs_universe_df):
    ...
    last_dist  = float(dist_sma20.iloc[-1]) if not np.isnan(dist_sma20.iloc[-1]) else 999.0
    ...
    # TIER2 filters
    if last_rvol < MIN_RVOL:       return None
    if adr_val < MIN_ADR:          return None
    if last_dist > MAX_DIST_SMA20: return None
    if dollar_vol < MIN_DOLLAR_VOL: return None
    if consol_days < MIN_CONSOL:   return None
    if float(volume.iloc[-1]) < MIN_VOLUME: return None
...
def main():
    ...
    print(f"  Config: tp1={T1['tp1_r']}R  tp2={T1['tp2_r']}R  "
    ...
    # Display
    print(f"\n{'='*70}")
    print(f"  SIGNALS FOR TOMORROW  |  {len(signals)} found from {len(universe)} scanned")
    print(f"{'='*70}")
    if signals:
        ...
        print("-" * len(hdr))
        for s in signals:
            print(f"{s['ticker']:<8} {s['entry_score']:>6.3f} {s['rs_percentile']:>5.1f} "
    ...
    print(f"  Risk: ${T1['risk_dollars']} | Exits: TP1={T1['tp1_r']}R/{int(T1['tp1_pct']*100)}%  "
